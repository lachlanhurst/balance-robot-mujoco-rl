...
"""
class Env03Fail(RobotBaseEnv):

    def __init__(self, **kwargs):
        RobotBaseEnv.__init__(self, 'env03_v1_fail.xml', **kwargs)

        self.block_delay_time_start = None
        # time between each block firing
        self.block_delay = 0.0

        self.dropping_fail = False
    ...
    def reset_model(self):
        ...
        self.dropping_fail = False
        self.data.joint('fail_joint').qpos[0:3] = np.array([10, 10, -2])
        self.data.joint('fail_joint').qvel[0:3] = np.array([0, 0, 0])
    ...
    def drop_fail(self):
        if self.dropping_fail:
            return
        self.dropping_fail = True
        robot_pos = self.data.body("robot_body").xpos
        fail_pos = np.array([robot_pos[0]-0.3, robot_pos[1]+0.3, 0.5])
        self.data.joint('fail_joint').qvel = np.array([0, 0, 0, 0, 0, 0])
        self.data.joint('fail_joint').qpos[3:7] = Rotation.from_euler('xyz', [math.pi/4,math.pi,math.pi/2 + math.pi]).as_quat()
        self.data.joint('fail_joint').qpos[0:3] = fail_pos
